...
public final class RootDaemon {
    ...
    private JSONObject observe() throws Exception {
        ...
        activePackage = value(root.getPackageName()); activeWindow = root.getWindowId();
        ...
        return new JSONObject().put("ok", true).put("package", activePackage).put("generation", generation)
    ...
    private synchronized JSONObject execute(JSONObject q) throws Exception {
        ...
        if (op.equals("switch_app")) {
            ...
            if (q.optBoolean("launch_if_missing",true)) {
                JSONObject launched = execute(new JSONObject().put("op","launch_app").put("package",pkg));
        ...
        boolean ok;
        if (op.equals("home") || op.equals("back")) { clear(); ok = ui.performGlobalAction(op.equals("home") ? 2 : 1); }
        else {
            int action;
            switch (op) {
            ...
            if (n == null || SystemClock.elapsedRealtime() - captured > 30000) throw new IllegalStateException("Stale node; observe again");
            ui.clearCache();
            AccessibilityNodeInfo root = ui.getRootInActiveWindow();
